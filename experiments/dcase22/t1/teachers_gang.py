...
def default_conf():
    ...
    # the path to the dataset should be changes in the dataset config
...
class M(Ba3lModule):
    def __init__(self, experiment):
        ...
        # value for mixup augmentation if this augmentation is used
        self.mixup_alpha = self.config.get("mixup_alpha", False)

        # value for mixstyle augmentation of mix style is used
        ...
        # Tempreture at wich the the soft targets of the teacher are calculated
        ...
        # if True network will be tested with quantized weights at the end of training
    ...
    # calculates only the melspectogram of the input for the student
    ...
    # calculates only the melspectogram of the input for the teacher
    ...
    def training_step(self, batch, batch_idx):
        ...
        # get input for the teacher
        x_teacher = self.teacher_mel_forward(x)
        x = self.mel_forward(x)
        batch_size = len(y)

        # augment input for teacher and student based on the augmentation method
        ...
        # logging results for the loss and accuracies
    ...
    # logging the training results at the end of epoch based on device
    ...
    def validation_step(self, batch, batch_idx):
        x, files, y, device_indices, cities, indices = batch
        # consistency with training: data is 10 second therefore they are split into 1 seconds and passed to network
        if self.random_sec:
            # x is of shape: batch_size, 1, 320000
            y = repeat(y, 'b -> (b 10)')
        ...
        # quantization only applied in validation step
    ...
    def validation_epoch_end(self, outputs):
        ...
        for net_name, net in model_name:
            ...
            val_acc = sum([x[net_name+'n_correct_pred'] for x in outputs]) * 1.0 / sum(x[net_name+'n_pred'] for x in outputs)
            logs = {'val.loss': avg_loss, 'val_acc': val_acc, 'step': self.current_epoch}

            for d in self.device_ids:
                dev_loss = torch.stack([x[net_name+"devloss." + d] for x in outputs]).sum()
                dev_cnt = torch.stack([x[net_name+"devcnt." + d] for x in outputs]).sum()
                dev_corrct = torch.stack([x[net_name+"devn_correct." + d] for x in outputs]).sum()
            ...
            self.log_dict({'step': self.current_epoch, **{net_name+k: v for k, v in logs.items()}})
    ...
    # function to get the logits of the network
...
# get prediction file of the teachers if they exist.
...
def get_pl_ckpt(ckpt=None, run_id=None, db_server=None, db_name=None):
    if ckpt is None:
        ...
        host_name = e['host']['hostname'].replace("rechenknecht", "rk").replace(".cp.jku.at", "")
        output_dir = "dcase22/malach_dcase22/" + e["config"]["trainer"]['default_root_dir']
        if "node" in host_name or "rk6" in host_name:
