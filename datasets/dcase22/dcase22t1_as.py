...
"""
this code is exactly like dcase22t1 but covers audioset as well.
"""
...
def default_config():
    ...
    # path to the cached files: on first run cached files will be saved here , else they will load from here
    cache_root_path = "/share/rk6/shared/kofta_cached_datasets/"
    base_dir = "/share/rk6/shared/dcase22/"  # base directory of the dataset as downloaded
    ...
    # path to calculated global norm file , firs run saves here else loads from here
...
class BasicDCASE22Dataset(TorchDataset):
    """
    Basic DCASE22 Dataset  output file, labeles devices and cities
    class input is meta_csv to retrieve file label device and cities
    """
...
class SpectrogramDataset(TorchDataset):
    ...
    return : spectogram (x) , file name, label, device and city
    """
...
class EvalSpectrogramDataset(TorchDataset):
    """
    gets the spectrogram from files using audioprocessor
    return : spectogram (x) and file name y is not available during evaluation
    """
...
# command to retrieve file from cached dataset
@dataset.command
